...
S32 GSC_CfgInit(GSC_CoreCfg_S *pstGscCfg, CONST CHAR *pcBinFn)
{
    ...
    if (iRet == -1)
    {
        AISP_TSL_printf("%s", "gscDefaultCfg init error.\n");
        goto FAIL;
    }
    ...
    if (iRet == -1)
    {
        AISP_TSL_printf("%s", "gscCfg update error.\n");
        goto FAIL;
    }
...
S32 GSC_CfgMemSizeGet(VOID)
{
    ...
    return iTotalMemSize + AISP_ALIGN_8_SIZE;
}
...
GSC_Api_S *GSC_New(GSC_CoreCfg_S *pstGscCfg, S8 *pcSharedBuf, S8 *pcUnsharedBuf)
{
    ...
    if (pcUnsharedBuf == NULL || pcSharedBuf == NULL)
    {
        return NULL;
    }
    ...
    pstGscApi->psHanningWind                = (GSC_S32_T *)AISP_TSL_get_win(WIN_HANNING_SQRT_512); /* FFT hanning window */
    ...
    iRet = fftInit(&pstGscApi->pstFFTEng, &pstGscApi->pstIFFTEng, GSC_FFTLEN);

    if (iRet < 0)
    {
        AISP_TSL_printf("%s", "FFT engine init fail.\n");
        goto FAIL;
    }
...
{
    pstGscApi->pfuncGscOutputHook = pvHandler;
    pstGscApi->pvUsrData          = pvData;
    return 0;
}
...
S32 GSC_Feed(GSC_Api_S *pstGscApi, CONST BYTE *pcData, S32 iLen)
{
    ...
    {
        AISP_TSL_printf("%s", "Input Length is not supported.\n");
        return iRet;
    }
    ...
    if (!iRet)
    {
        ...
        else
        {
            ...
            if (pstGscApi->pfuncGscOutputHook)
            {
                ...
                {
#ifdef LUDA_SUPPORT_FREQOUT_DIRECT
                    pstGscApi->pfuncGscOutputHook(pstGscApi->pvUsrData, iIdx, (BYTE *)aiZero, GSC_FFTBINS * sizeof(GSC_COMPLEX_T));
#else
                    pstGscApi->pfuncGscOutputHook(pstGscApi->pvUsrData, iIdx, (BYTE *)asZero, GSC_FRMINC << 1);
#endif
                }
            }
        }
...
S32 GSC_Reset(GSC_Api_S *pstGscApi)
{
    ...
    {
        AISP_TSL_printf("%s", "GSC_Reset: NULL pointer.\n");
        return -1;
    }
...
S32 GSC_Delete(GSC_Api_S *pstGscApi)
{
    if (pstGscApi != NULL)
    {
        AISP_fft_delete(pstGscApi->pstFFTEng);
        AISP_fft_delete(pstGscApi->pstIFFTEng);
        return 0;
    }
